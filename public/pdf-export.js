...
(function (ns) {
  ...
  ns.drawFooter = function drawFooter(doc, pageNumber, pageCount) {
    ...
    const footer = `Generated by Credit Commander   Page ${pageNumber} of ${pageCount}`;
    ...
    const disclaimer = "Advisory: For educational purposes only. Consult a licensed credit professional before taking action.";
...
(() => {
  ...
  function getToolkitName() {
    return window.currentToolkitName || "Credit Commander";
  }

  function getToolkitIcon() {
    // Use the full-size logo for PDF exports (not the tiny favicon)
    // TODO: Add Credit Commander logo asset
    return window.currentToolkitLogo || null;
  }
  ...
  // ---- Header Drawing ------------------------------------------------------
  function drawHeader(doc, pageNum, iconDataUrl, metadata = {}) {
    const x = MARGINS.left;
    const y = MARGINS.top;
    
    // Credit Commander logo with proper aspect ratio (square logo 1015x1001 = 1:1)
    ...
    // Date (right-aligned) - use nullish coalescing to preserve empty strings
    const dateText = metadata.date ?? new Date().toLocaleDateString('en-US', { 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric' 
    });
    ...
    doc.text(dateText, CONTENT.right - dateWidth, y + 7);
    
    // Summary Metrics (below title, if available) - use nullish checks to preserve 0 values
    if (metadata.fico != null || metadata.utilization != null || metadata.tradeLines != null) {
      const metricsY = y + 11;
      doc.setFont(TYPOGRAPHY.fontFamily, "normal");
      doc.setFontSize(8);
      ...
      let metricsText = '';
      if (metadata.fico != null) metricsText += `FICO: ${metadata.fico}`;
      if (metadata.utilization != null) metricsText += (metricsText ? ' • ' : '') + `Utilization: ${metadata.utilization}%`;
      if (metadata.tradeLines != null) metricsText += (metricsText ? ' • ' : '') + `Trade Lines: ${metadata.tradeLines}`;
      
      doc.text(metricsText, x + logoSize + 4, metricsY);
    }
  ...
  async function drawFooter(doc, pageNum, totalPages) {
    ...
    doc.text("Generated by Credit Commander", CONTENT.left, mainFooterY);
  ...
  class PDFWriter {
    constructor(doc, iconDataUrl, metadata = {}) {
      this.doc = doc;
    ...
    addNewPage() {
      ...
      this.pageNum = window.YBG_PDF.safeAddPage(this.doc, this.pageNum, (pageNum) => {
        ...
        drawHeader(this.doc, pageNum, this.iconDataUrl, this.metadata);
      });
  ...
  async function exportSingleResult(text, resultTitle) {
    ...
    const filename = `CreditCommander_${yyyy}-${mm}-${dd}_CreditRoadmap.pdf`;
  ...
  async function exportMultipleResults(mode = 'all') {
    ...
    const filename = `CreditCommander_${yyyy}-${mm}-${dd}_${suffix}.pdf`;
    ...
    function processNode(node) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        if (node.classList && node.classList.contains('doc-meta')) {
          // Skip doc-meta in PDF - already shown in KPI dashboard
          return;
        }
        ...
        switch (tagName) {
          ...
          case 'h4':
            // Card names should be bold paragraphs (not gold) for PDF readability
            if (node.classList && node.classList.contains('card-name-title')) {
              markdown += '\n**' + node.textContent.trim() + '**\n';
            } else {
              markdown += '\n' + node.textContent.trim() + '\n';
            }
          ...
          case 'hr':
            // Skip HR in PDF - section headings provide enough separation
            break;
  ...
  // ---- Native PDF Rendering for Visual Elements (Clean, No Screenshots) ----
  ...
  function extractKPIData() {
    ...
    return {
      utilization: utilizationEl ? utilizationEl.textContent.trim() : '--',
      tradelines: tradelinesEl ? tradelinesEl.textContent.trim() : '--',
      fico: ficoEl ? ficoEl.textContent.trim() : '--'
    };
  }
  
  function extractSignalsData() {
    const signalsStrip = document.getElementById('signalsStrip');
    if (!signalsStrip || signalsStrip.style.display === 'none') {
      return null;
    }
    
    const signals = [];
    const badges = [
      { id: 'badgeUtilization', label: 'Last 6mo Utilization' },
      { id: 'badgeLatePayments', label: 'Late Payments' },
      { id: 'badgeDerogatories', label: 'Derogatories' },
      { id: 'badgeFico', label: 'FICO' },
      { id: 'badgeTradeLines', label: 'Trade Lines' }
    ];
  ...
  function renderKPIInPDF(doc, yPosition, kpiData) {
    const boxHeight = 50;
    const boxMargin = 4;
    const boxWidth = (CONTENT.width - boxMargin * 2) / 3;
    
    // Light background for clean look
    doc.setFillColor(250, 250, 252);
    doc.roundedRect(CONTENT.left, yPosition, CONTENT.width, boxHeight, 3, 3, 'F');
    
    // Border
    doc.setDrawColor(220, 220, 230);
    doc.setLineWidth(0.5);
    doc.roundedRect(CONTENT.left, yPosition, CONTENT.width, boxHeight, 3, 3, 'S');
    
    // Title
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(60, 60, 80);
    doc.text('Credit Metrics Overview', CONTENT.left + 5, yPosition + 8);
    
    // Three KPI boxes
    const kpis = [
      { label: 'Credit Utilization', value: kpiData.utilization, color: [77, 182, 231] },
      { label: 'Trade Lines', value: kpiData.tradelines, color: [255, 213, 74] },
      { label: 'Owner FICO', value: kpiData.fico, color: [77, 182, 231] }
    ];
    
    kpis.forEach((kpi, index) => {
      const x = CONTENT.left + 5 + (boxWidth + boxMargin) * index;
      ...
      doc.text(kpi.label, x, boxY);
      
      // Value
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(18);
      doc.setTextColor(...kpi.color);
      doc.text(kpi.value, x, boxY + 12);
    });
    
    return yPosition + boxHeight + 8;
  }
  
  function renderSignalsInPDF(doc, yPosition, signalsData) {
    ...
    // Light background
    doc.setFillColor(250, 250, 252);
    doc.roundedRect(CONTENT.left, yPosition, CONTENT.width, badgeHeight, 3, 3, 'F');
    
    // Border
    doc.setDrawColor(220, 220, 230);
    doc.setLineWidth(0.5);
    doc.roundedRect(CONTENT.left, yPosition, CONTENT.width, badgeHeight, 3, 3, 'S');
    
    // Title
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.setTextColor(60, 60, 80);
    doc.text('Signals at a Glance:', CONTENT.left + 5, yPosition + 6);
    
    // Badges inline
    let xPos = CONTENT.left + 50;
    const badgeYCenter = yPosition + 6;
    
    signalsData.forEach((signal, index) => {
      // Badge background
      doc.setFillColor(240, 240, 245);
      doc.roundedRect(xPos, yPosition + 3, 28, 10, 2, 2, 'F');
      
      // Badge border
      doc.setDrawColor(200, 200, 210);
      doc.setLineWidth(0.3);
      doc.roundedRect(xPos, yPosition + 3, 28, 10, 2, 2, 'S');
      
      // Badge text
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8);
      doc.setTextColor(60, 60, 80);
      const textWidth = doc.getTextWidth(signal.value);
      doc.text(signal.value, xPos + 14 - textWidth / 2, badgeYCenter + 2);
      
      xPos += 30;
    });
    
    return yPosition + badgeHeight + 8;
  }
  ...
  window.exportAllResultsToPDF = function(resultsArray, options = {}) {
    ...
    if (Array.isArray(resultsArray) && resultsArray.length > 0 && resultsArray[0]?.html) {
      ...
      const customFilename = item.fileName || `CreditCommander_${Date.now()}.pdf`;
      
      // Extract metadata for header
      const metadata = item.metadata || {};
      
      // Export with custom filename and metadata
      return exportSingleResultWithFilenameAndVisuals(markdownContent, customFilename, metadata);
    }
  ...
  // Helper: Export with custom filename and visual elements
  async function exportSingleResultWithFilenameAndVisuals(text, filename, metadata = {}) {
    const jsPDF = await loadJsPDF();
    const doc = new jsPDF({ unit: 'mm', format: 'a4' });
    ...
    // Extract visual elements data (no screenshots needed)
    console.log('PDF Export: Extracting visual elements data...');
    const kpiData = extractKPIData();
    const signalsData = extractSignalsData();
    
    // Initialize writer with metadata
    const writer = new PDFWriter(doc, iconDataUrl, metadata);
    
    // Apply safe body style
    window.YBG_PDF.applyBodyStyle(doc);
    
    // Draw watermark and header on first page with metadata
    drawWatermark(doc, iconDataUrl);
    drawHeader(doc, 1, iconDataUrl, metadata);
    
    // Render KPI panel natively in PDF (clean, light design)
    if (kpiData) {
      console.log('PDF Export: Rendering KPI panel natively');
      writer.yPosition = renderKPIInPDF(doc, writer.yPosition, kpiData);
    }
    
    // Render signals badges natively in PDF (clean, light design)
    if (signalsData) {
      console.log('PDF Export: Rendering signals badges natively');
      writer.yPosition = renderSignalsInPDF(doc, writer.yPosition, signalsData);
    }
  ...
  // Helper: Export with custom filename (legacy, without visuals)
  async function exportSingleResultWithFilename(text, filename, metadata = {}) {
    const jsPDF = await loadJsPDF();
    const doc = new jsPDF({ unit: 'mm', format: 'a4' });
